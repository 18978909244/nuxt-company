  ...
  <el-row :gutter="20">
    ...
    <el-table-column
      ...
      <template slot-scope="scope" v-if="scope.row.order_status!==9">
        ...
        <el-button type="text" size="small" v-if="scope.row.order_status===3">支付</el-button>
...
export default {
      ...
      getOrderList:function(){
        return this.typeOrderList[this.index].orderList.map(item=>{
          return {
            ...
            service_name:item.product && item.product.product_and_service && item.product.product_and_service.service_name
          }
        })
      }
  },
  ...
  methods: {
    ...mapActions(['asyncCancelOrder', 'asyncPayOrder', 'asyncDeleteOrder','initMyOrder']),
